...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error('Commits per Repo API Error:', error);
    
    // Check if it's a rate limit error
    if (error instanceof Error && error.message.includes('rate limit exceeded')) {
      // Return fallback data when rate limited
      const fallbackRepos: RepoData[] = [
        { name: 'awesome-project', value: 234, color: GREEN_COLOR_ARRAY[0] },
        { name: 'cool-app', value: 189, color: GREEN_COLOR_ARRAY[1] },
        { name: 'data-analysis', value: 156, color: GREEN_COLOR_ARRAY[2] },
        { name: 'web-tools', value: 123, color: GREEN_COLOR_ARRAY[3] },
        { name: 'api-service', value: 98, color: GREEN_COLOR_ARRAY[4] },
        { name: 'mobile-app', value: 87, color: GREEN_COLOR_ARRAY[5] },
        { name: 'cli-tool', value: 76, color: GREEN_COLOR_ARRAY[6] },
        { name: 'game-engine', value: 65, color: GREEN_COLOR_ARRAY[7] },
        { name: 'ml-model', value: 54, color: GREEN_COLOR_ARRAY[8] },
        { name: 'dashboard', value: 43, color: GREEN_COLOR_ARRAY[9] },
      ];
      ...
      const chartWidth = width - margin.left - margin.right;
      const chartHeight = height - margin.top - margin.bottom;
      const barWidth = chartWidth / fallbackRepos.length * 0.7;
      const barSpacing = chartWidth / fallbackRepos.length * 0.3;
    ...
    const errorSvg = `
      <svg width="600" height="400" xmlns="http://www.w3.org/2000/svg">
        <rect width="100%" height="100%" rx="4" fill="${RETRO_COLORS.DARK_BG}" stroke="${RETRO_COLORS.BORDER_COLOR}" stroke-width="2"/>
        <text x="300" y="200" text-anchor="middle" fill="${RETRO_COLORS.CYAN}" font-size="14" font-family="'Determination', 'Retro Gaming', monospace">
          Error loading data
        </text>
      </svg>
    `;
